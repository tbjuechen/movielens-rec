...
RANK_DIN_ATTENTION_HIDDEN = _ranking.get('din_attention_hidden', [64, 32])
...
RANK_NEGATIVES_PER_POSITIVE = _ranking.get('negatives_per_positive', _ranking.get('neg_sample_ratio', 3))
RANK_HARD_NEGATIVE_TOPK = _ranking.get('hard_negative_topk', 20)
RANK_EXPLICIT_NEGATIVE_THRESHOLD = _ranking.get('explicit_negative_threshold', 3.0)
_hard_negative_mix = _ranking.get('hard_negative_mix', [2, 1])
if not isinstance(_hard_negative_mix, list) or len(_hard_negative_mix) != 2:
    _hard_negative_mix = [2, 1]
